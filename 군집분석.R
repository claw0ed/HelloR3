...
# 군집 분석의 유사성(두 개체가 가까우면)은 거리척도/상관계수를 이용
...
library(cluster)
# dist(academy, method='')
# euclidean, maximum, manhattan, canberra, binary, minkowskia
...
# 비계층적 군집방법 : k-means 군빚
kms4 <- kmeans(academy, 4);kms4 #4개 군집
kms5 <- kmeans(academy, 5);kms5 #5개 군집
kms6 <- kmeans(academy, 6);kms6 #6개 군집
