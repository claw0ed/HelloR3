...
# 총면적 모양은 항상 일정하지 않을 수 있다
# 균등분포는 사각형 모형, 정규분포는 종 보양을 이룸
...
x <- c(-2:20)
ggplot(x, aes(x=x)) +
  stat_function(fun_dunif, args=list(0,10),
                colour='black', size=1) +
  ggtitle('최소값1, 최대값10의 균등분포') # 안됨 나중에
...
# 173cm 를 크게 벗어나지 않는 사람들이 많고
# 상대적으로 크게 벗어난 150cm나 190cm대의
# 사람들은 별로 없다는 의미임

# 정규분포는 평균근처에서 데이터의 분포가 많고
...
# 평균이 50, 표준편차가 4인 정규분포로부터 난수 100개를 발생
...
polygon(c(-1.5, x2), c(0, y2), col='red') # x2, y2를 기준으로 -1.5이상인 정규분포의 면적을 그림
...
# 평균을 중심으로 데이터들의 흩어집 정도 - 분산
# 이렇게 퍼져있는 분산의 모양울 보고 분포로 작성 -
# 카이제곱 (분산의 제곱된 값을 다룸)
...
# F분포
# 카이제곱 분포와 유사하게 집단의 분산을 추정하고 검증시 사용
# - 제곱값을 다루기 때문에 +값만 존재

# 한편, 카이제곱은 한 집단의 분산을 파악할때 사용하지만
# F분포는 두 집단간 분산을 파악할때 사용
# 카이제곱 분포의 향상된 버전으로 봐도 된다
